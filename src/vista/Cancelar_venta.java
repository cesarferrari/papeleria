...
public class Cancelar_venta extends javax.swing.JFrame {
   ...
   // List<String>llenado= new ArrayList<>();
    ...
    public void fill(){
          String []llenado=new String[codigos(codigo_venta()).size()];
             String []cantidad=new String[codigosConteo(codigo_venta()).size()];
      for (int i = 0; i < llenado.length; i++) {
         llenado[i]= codigos(codigo_venta()).get(i);
          cantidad[i]= codigosConteo(codigo_venta()).get(i);
      }
        for (int i = 0; i < llenado.length; i++) {
  ...
  public void llenado(){
   ...
   public String codigo_venta(){
       ...
       try{
           Statement st=cnx.createStatement();
           ResultSet rs=st.executeQuery("select codigo,productos from venta where codigo="+this.txt_codigo.getText());
           while (rs.next()) {
   ...
   public List<String>codigos(String codigo){
      ...
      //int totales=this.Tabla.getRowCount();
          ...
          PreparedStatement pst=cnx.prepareStatement("update venta set productos='"+productoCancelado()+"' where codigo="+this.txt_codigo.getText());
      ...
      int totales=this.Tabla.getRowCount();
          ...
          PreparedStatement pst=cnx.prepareStatement("update venta set productos='"+productosCanceladoCiclo()+"' where codigo="+this.txt_codigo.getText());
    ...
    private void initComponents() {
        ...
        btn_all.setText("cancelar toda la venta");
    ...
    private void btn_buscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_buscarActionPerformed
     //   System.out.println(codigosConteo(codigo_venta()));
       // System.out.println(codigos(codigo_venta()));
      limpia_tabla();
